...
public class AGSAdapter {
    ...
    public static final PointF[] directPointList = new PointF[8];
    ...
    public synchronized void show(
            ...
            boolean isLoop,
            Point focusPoint) {
        String direct_str = isGoods ? "" : getDirect(direct, 8);

        PicAreaArrayItf paa = resList.get(name + direct_str);
        if (paa == null) {
            ErrorDeal.putError("error happened in AgsAdapter, name not exists: " + name + direct_str + "|");
            return;
        }
    ...
    public void show(
        ...
        if (name == null || name.equals("")) {
            throw new Exception(
                    "error happened in AgsAdapter, state-name pear not exists: action name is: "
                            + name
                            + "  , type name is: "
                            + type
                            + "  , type id is: "
    ...
    public String getDirect(
            PointF pf, int direct_nums) { // direct_nums == (8 or 4) if not equ 8  then think eqa 4
