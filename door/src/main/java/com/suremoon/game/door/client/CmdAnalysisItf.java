...
public interface CmdAnalysisItf {
   ...
   */
  default boolean addState(int cmdType) {
    return addState(getActionList(), cmdType);
  }
  ...
  default ActionData[] oldErrorDataTrans(int[] inp) {
    ...
    for (int i = 0, j = 0; i < inp.length; ++i) {
      if (inp[i] < 0) {
        ...
        res[j++] =
            new ActionData(ActionData.DATA_TYPE_MOUSE, -inp[i], ActionData.VALUE_TYPE_REPLASE);
      } else {
        res[j++] =
            new ActionData(ActionData.DATA_TYPE_KEY_BOARD, inp[i], ActionData.VALUE_TYPE_PRESS);
        res[j++] =
            new ActionData(ActionData.DATA_TYPE_KEY_BOARD, inp[i], ActionData.VALUE_TYPE_REPLASE);
      }
    }
