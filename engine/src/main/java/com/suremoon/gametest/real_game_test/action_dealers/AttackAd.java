...
public class AttackAd implements CmdActionItf {
    ...
    public boolean actionDo(CommandItf cmd, WorldItf world, WorldMgrItf worldMgr) {
        ...
        int length = 5;
//        int length = ui.getShower().resList.get(ui.getShower().getState2action().get(IDManager.getName(p.getState().getAGType())) + " e").getLength();
//        if(idx >= length){
...
//        }
        ...
        Rectangle impactArea = new Rectangle((int)(p.getFootPos().X - UNIT_MAX_WIDTH), (int)(p.getFootPos().Y -
                UNIT_MAX_WIDTH), UNIT_MAX_WIDTH*2, UNIT_MAX_WIDTH*2);
        gm.getUnitMgr().unitsDo(impactArea, unit -> {
            if (unit == p) return  true;
            if (getDistance(p, unit) <= p.getWidth() / 2.0 * 1.5) {
                if (PointF.getDistance(p.getDirect(), p.getFootPos().getDirection(unit.getFootPos())) <= 1.732 / 2.0) {
