...
public class FireHoneActionDo implements EffectActionItf {
  ...
  public void effectDo(EffectItf effect, WorldItf world, WorldMgrItf wm) {
    ...
    gm.getUnitMgr()
        .unitsDo(
            new Rectangle(
                rect.x - rect.width, rect.y - rect.height, rect.width * 3, rect.height * 3),
            (UnitItf u) -> {
              if (u != puter) {
                if (u.toRect().intersects(effect.toRect())) {
                  ...
                  u.underAttack(
                      puter,
                      effect.getHurt_ad() * puter.getAttribute().getMetal(),
                      effect.getHurt_ap() * puter.getAttribute().getFire());
