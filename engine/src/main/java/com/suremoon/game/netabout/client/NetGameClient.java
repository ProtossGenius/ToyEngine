...
public class NetGameClient extends Thread{
        ...
        while(running){
            PieceRun.DoPeaceRun(15, ()->{
                try{
                    ...
                    byte[] msg = CJDeal.ByteArrayConnect(new byte[]{predef.ScreenReq}, new MsgScreenAsk(screen).toBytes());
                    os.write(CJDeal.int2byte(msg.length));
                    os.write(msg);
                    os.flush();
                }catch (IOException e){
    ...
    public Player login(String unitType, String showName) throws IOException {
        ...
        byte[] msg = CJDeal.ByteArrayConnect(new byte[]{predef.LoginReq},  CJDeal.int2byte(v1.length), v1, CJDeal.int2byte(v2.length), v2);
        write(msg);
