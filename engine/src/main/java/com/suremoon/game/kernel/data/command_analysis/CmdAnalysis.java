...
public class CmdAnalysis implements CmdAnalysisItf {
  ...
  public void putAction(ActionData nType) {
    if (actions + 1 == actionList.length) dropNAction(1);
  ...
  public boolean addState(ActionData[] inp, CmdCreatorItf cmdCreator) {
    if (inp.length > 10) {
      throw new IndexOutOfBoundsException(
          "set actionList's max deep is 10. In CmdAnalysis::addState");
    }
    ...
    public boolean do_action(CANode caNode, int index) {
      ...
      for (int i = 0; i < nodes.size(); ++i) {
        ...
        if (node.getCmdId().equals(atype)) {
          ...
          if (canDelete) {
            if (node.getNext().size() == 0 || node.isLeaf()) nodes.remove(node);
          }
          return canDelete;
        }
      }
  ...
  protected class GetInfJI implements JudgeInf {
    ICPair icp;
    /**
     * todo 此处可以优化 2018/3/23 dropableSize 是指目前得到的事件队列并不能够匹配成功,但是在某些条件下却可以判断一些旧的命令可以抛弃掉
    ...
    public boolean do_action(CANode caNode, int index) {
      ...
      for (int i = 0; i < nodes.size(); ++i) {
        ...
        if (cn.getCmdId().equals(atype)) {
          ...
          /**
           * else if(atype != KeyEvent.VK_CONTROL && atype != KeyEvent.VK_SHIFT && atype !=
           * KeyEvent.VK_ALT){ 如果不是这三个特殊的按键，则说明 return false;
      ...
      // if exist the node whose cmdId == that id then can't goto there, so there should create
      // node.
  ...
  interface JudgeInf {
    ...
    /**
     * for(int i = 0; i < caNode.getNext().size(); ++i){ if(can){ do sth.
     * deepLook(caNode.getNext().get(i), this); }else{ do another thing. } }
     */
  }
}
