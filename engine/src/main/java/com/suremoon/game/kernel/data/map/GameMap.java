...
//    protected Point lastPoint = new Point(0, 0);
    ...
    public GameMap(Terrain[][] map, int width,  int height) {
        ...
        this.em = new GRMEffectMgr(new Dimension(totalWidth, totalHeight), new Dimension(width*5, height*5));
        this.um = new GRMUnitMgr(new Dimension(totalWidth, totalHeight), new Dimension(width*5, height*5));
    }
    ...
    static Point[] nexts = new Point[]{new Point(-1, 0), new Point(1, 0), new Point(0, -1), new Point(0, 1)};
    private void fillConnectivity(int x, int y, int no){
        ...
        while(!q.isEmpty()){
            ...
            for(Point it : nexts){
                ...
                if(isLegalPos(nextX, nextY) && connectivity[nextX][nextY] == null && getTerrain(nextX, nextY).walkAble(null)){
                    ++num;
                    connectivity[nextX][nextY] = new Connectivity(frontConn.blockPos, frontConn.branchFlag, true, no);
    ...
    private void checkConnectivity(){
        for(int i = 0; i < connectivity.length; ++i){
            ...
            for(int j = 0; j < connectivity[i].length; ++j){
                if(connectivity[i][j] == null){
                    System.err.println("Error, connectivity init fail, connectivity[" + i + "][" + j + "] == null");
                    break;
                }
            }
        }
    ...
    public void changeMap(MapInformation mapIfm) {
        ...
        totalWidth = width * cols;
        totalHeight = height * rows;
        this.em = new GRMEffectMgr(new Dimension(totalWidth, totalHeight), new Dimension(width, height));
