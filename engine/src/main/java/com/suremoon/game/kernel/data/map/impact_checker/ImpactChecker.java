...
public class ImpactChecker implements ImpactCheckerItf {
    ...
    class CanMove implements UnitDoItf{
        ...
        public boolean Do(UnitItf other) {
            ...
            if (targetDistance < nowDistance && targetDistance < (double) (Math.min((unit.getWidth() + other.getWidth()) / 6, (unit.getHeight() + other.getHeight()) / 3))){
                canMove = false;
                return  false;
            }
    ...
    public boolean UnitMovable(UnitItf unit, GameMapItf gameMap, PointF aims){
        ...
        if(toPos.x <= 0 || toPos.x >= gameMap.getMapWidth() || toPos.y <= 0 || toPos.y >= gameMap.getMapHeight()){
            return false;
        }
        canMove.reset(unit, toPos);
        Rectangle impactArea = new Rectangle((int)(unit.getFootPos().X - UNIT_MAX_WIDTH), (int)(unit.getFootPos().Y -
