...
public class StandIResConf {
    ...
    public StandIResConf(String confPath) throws ParserConfigurationException, NoSuchConfigException, SAXException, TransformerConfigurationException, IOException, NotLeafException {
    ...
    public StandIResConf(ConfigInf ec) throws NoSuchConfigException,  NotLeafException{
        ...
        confLists[0] = new ResConfNode(0, smi, "Null", new Point(0, 0), new Point(smi.getImageWidth(), smi.getImageHeight()));
        for(int i = 0; i < cis.length; ++i){
            String sid = getValue(cis[i], "Id"), start = getValue(cis[i], "StartPoint"), end = getValue(cis[i], "EndPoint"), describe = getValue(cis[i], "Describe");
    ...
    public final static Point toPoint(String inp){
        String[] xy = inp.split(",");
        xy[0] = xy[0].trim();
        xy[1] = xy[1].trim();
