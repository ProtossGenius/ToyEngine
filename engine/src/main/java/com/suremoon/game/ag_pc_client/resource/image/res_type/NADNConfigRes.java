...
public class NADNConfigRes
    implements ConfigResInf { // --> '_' means space.like ->|name e0001.bmp|<-
    ...
    for (int i = 0; i < length; i++) {
      rt[i] =
          base_path
              ...
              + " "
              + direct
              + String.format("%0" + num_length + "d", i)
              + "."
              + ext;
    }
    return rt;
  ...
  public String[][] getResList(
      ...
      int length,
      int num_length,
      String ext) { // directs =is= (4 or 8), another think is 4
    if (directs == 1) {
