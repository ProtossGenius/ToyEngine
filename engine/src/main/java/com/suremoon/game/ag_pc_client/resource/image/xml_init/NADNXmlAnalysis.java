  ...
  public void analysis(ConfigInf conf, Map<String, PicAreaArrayItf> res, String base_path)
    ...
    for (ConfigInf ci : cis) {
      String name = ci.getValue("name"),
          action_nums = ci.getValue("action_nums"),
          directs = ci.getValue("directs"),
          numlength = ci.getValue("numlength");
      NADNResourceIniter.initResource(
          base_path,
          trans,
          Integer.valueOf(directs),
          res,
          Integer.valueOf(numlength),
          name,
          Integer.valueOf(action_nums),
          ext);
    }
  }
