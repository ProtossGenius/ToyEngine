...
public class StringShow {
    ...
    LinkedList<String> ssls = new LinkedList<>();
    LineSpacing ls = new FixedLineSpacing();
    int charHeight = 1;
    int startIndex = 0;

    public StringShow(String str, Rectangle showArea){this(str, showArea, new Font("宋体", Font.BOLD, 24));}
    ...
    protected void drawDeal(FontMetrics fm, int height) {
        ...
        for (int i = pos; i < copy.length(); i++) {
            ...
            int nextLns = fm.stringWidth(tmp + c);
    ...
    public boolean drawOn(Graphics gp, int yChange){
        ...
        for(Iterator<String> iter = ssls.iterator(); iter.hasNext(); ){
            ...
            int lineY = showArea.y + index * charHeight +yChange;
            if(lineY < showArea.y) {
                iter.remove();
                ++startIndex;
