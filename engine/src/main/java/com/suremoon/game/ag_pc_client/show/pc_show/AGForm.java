...
public class AGForm extends JFrame implements ScreenInfoUpdateItf {
    ...
    public AGForm(String path, int index) throws Exception {
        ...
        rollingString = new RollingString(
                new StringShow(
                        "",
                        new Rectangle(GameConfiger.gc.DESIGN_SCREEN_WIDTH - 500, 50, 500, GameConfiger.gc.DESIGN_SCREEN_HEIGHT-50)
                )
        ) ;
    ...
    public void gameLoop() {
        ...
        while(true) {
            try {
                ...
                StringShow clientInfo = new StringShow("GRect num: " + (msgs[1].length + msgs[2].length) + "\n" +
                        "Fps : " + fps.See() + "\n" +
                        "Nps : " + nps.See(), new Rectangle(30, 130, 200, 800));
                clientInfo.drawOn(cache);
                Point mousePos = mkid.getLastPoint();
                StringShow showMousePos = new StringShow("mouse's real pos = (" + mousePos.x + ", " + mousePos.y + ")",
