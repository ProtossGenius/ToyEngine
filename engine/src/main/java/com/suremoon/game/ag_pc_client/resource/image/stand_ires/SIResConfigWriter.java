...
public class SIResConfigWriter {
    ...
    for (int i = 0; i < rcns.size(); ++i) {
      ...
      fw.write(
          "\t\t<ResConfig Id = \""
              ...
              + rc.getDescribe()
              + "\" StartPoint = \""
              + PointToString(rc.getStart())
              + "\" EndPoint = \""
              + PointToString(rc.getEnd())
              + "\"/>\n");
    }
    fw.write("\t</ResList>\n");
    fw.write("</Root>");
  ...
  //        SIResConfigWriter sircw = new SIResConfigWriter("resource/Effect/FlowerFire.xml",
  // "resource/Effect/FlowerFire.jpg");
  //        //6, 5
  //        int ptr = 1;
  //        for(int i = 0; i < 6; ++i){
  //            for(int j = 0; j < 5; ++j){
  //                StandIResConf.ResConfNode rcn = new StandIResConf.ResConfNode(ptr, img,
  // "no-need", new Point(width * j / 5, height * i / 6), new Point(width * (j+1) / 5, height *
  // (i+1) / 6));
  //                ++ptr;
