...
public class ClientStartup {
    public static void defaultCmdAnalysis(CmdAnalysisItf ca){
        ...
        ca.addState(new ActionData[]{new ActionData(0, KeyEvent.VK_A, VALUE_TYPE_PRESS),
                new ActionData(DATA_TYPE_MOUSE, MouseEvent.BUTTON1, VALUE_TYPE_PRESS),
                new ActionData(DATA_TYPE_MOUSE, MouseEvent.BUTTON1, VALUE_TYPE_REPLASE)}, "CmdAttack");
        ca.addState(new ActionData[]{new ActionData(DATA_TYPE_KEY_BOARD, KeyEvent.VK_A, VALUE_TYPE_PRESS),
                new ActionData(DATA_TYPE_MOUSE, MouseEvent.BUTTON1, VALUE_TYPE_PRESS),
    ...
    public static void singleClient(String worldMgrCfg, String playerType, String playerName, CmdAnalysisInitItf analysisClass) throws Exception {
        WorldMgr wm = new WorldMgr(worldMgrCfg);
        WorldItf world = wm.getWorld(0);
        if (world == null){
            throw new Exception("no such world whose index = 0");
        }
        PlayerItf player = new Player((Unit) UnitInfManager.getUim().productUnit(IDManager.getID(playerType)));
    ...
    public static void networkClient(String serverAddress, int port, String worldMgrCfg, String playerType, String showName, CmdAnalysisInitItf analysisClass) throws Exception {
