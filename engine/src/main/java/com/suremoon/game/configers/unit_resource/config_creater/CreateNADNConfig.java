...
public class CreateNADNConfig {
    ...
    protected void dealDirectory(LinkedList<NADN> lln, File f) throws IOException {
        ...
        for(int i = 0; i < fils.length; ++i){
            ...
            if (!action_name.equals(an) || i == fils.length-1){
                lln.add(new NADN(action_name, Integer.toString(Integer.valueOf(length)+1), Integer.toString(directs), Integer.toString(tl.length())));
                action_name = an;
            }
            length = tl;
        }
    ...
    public void toXml() throws IOException {
        ...
        fw.write("\t<Actions>\n");
        for(NADN it: nadn){
            fw.write("\t\t<action name=\"" + it.name + "\" action_nums=\"" + it.action_nums + "\" directs=\"" + it.directs + "\" numlength=\"" + it.num_length + "\"/>\n");
        }
        fw.write("\t</Actions>\n" +
                "</Root>\n");
        fw.flush();
